...
def fetch_latest(limit=100):
    ...
    try:
        ...
        for page in range(1, math.ceil(limit/50)+1):
            ...
            for it in items:
                try:
                    results.append({"period": it["issueNumber"], "number": int(it["number"])})
                except:
                    pass
...
def predict_number(seq, pred_size):
    ...
    for n, v in sorted(comb.items(), key=lambda x: -x[1]):
        ...
        if r <= cum:
            return n
    return random.choice([5,6,7,8,9]) if pred_size=="Big" else random.choice([0,1,2,3,4])
    ...
    while True:
        try:
            ...
            for period in get_pending():
                ...
                settle_msg = (f"{emoji} *RESULT SETTLED — #{period}*\n"
                ...
                if CHAT_ID:
                    try:
                        await app.bot.send_message(chat_id=CHAT_ID, text=settle_msg, parse_mode="Markdown")
                    except:
                        pass
...
async def cmd_predict(upd: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not data or len(data) < 15:
        await upd.message.reply_text("⚠️ API unavailable. Try again in a moment.")
        return
    for item in reversed(data):
        log_history(item["period"], item["number"])
    cp = data[0]["period"]
...
async def cmd_stats(upd: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for L in range(1,7):
        ...
        t = d["same"] + d["opp"]
        if t > 0:
            sl += f"Streak-{L}: rev {round(d['opp']/t*100)}% ({t} obs)\n"
    await upd.message.reply_text(
        f"📊 *BOT STATISTICS*\n"
        f"━━━━━━━━━━━━━━━━━━━\n"
...
async def cmd_history(upd: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    conn.close()
    if not rows:
        await upd.message.reply_text("No history yet.")
        return
...
async def cmd_accuracy(upd: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    conn.close()
    if not rows:
        await upd.message.reply_text("No settled predictions yet.")
        return
    lines = []
...
async def cmd_streak(upd: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not data:
        await upd.message.reply_text("Could not fetch data.")
        return
    seq = [item["number"] for item in data]
    streak = 1
    lc = get_class(seq[0])
